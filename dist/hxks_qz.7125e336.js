...
checkBtn.addEventListener('click', function (e) {
  ...
  var selectedAnswers = document.querySelectorAll('input[type=radio]:checked'); // selectedAnswers.forEach((answer) => {
  ...
  if (availableAnswers.length == selectedAnswers.length) {
    ...
    (function () {
      ...
      for (var _i = 0; _i < selectedAnswers.length; _i++) {
        if (selectedAnswers[_i].value === _hxks.default[_i].correct) {
          countTrue += 1;
          availableAnswers[_i].style.border = '4px solid rgba(19, 111, 99, 0.3)';
        } else {
          countFalse += 1;
          availableAnswers[_i].style.border = '4px solid rgba(162, 44, 41, 0.3)';
